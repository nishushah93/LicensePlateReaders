...
//	struct plateInfo * plate_info = malloc( sizeof(plateInfo) * 8 );
	...
	char * plate_buffer[] = {"plate0.jpg", "plate1.jpg", "plate2.jpg",
		...
		"plate7.jpg", "plate9.jpg", "plate10.jpg"};


	//This should be 8
	*numContoursP = cvFindContours( resize_crop, storage, &cvSeq,
	...
	//Now get the characters (using stuff from commented section below
	struct plateInfo * plate_info = malloc( sizeof(plateInfo) *
			(*numContoursP) );
  
	for( i = 0; i < *numContoursP; i++)
	{
		//Get bounding rect
...
*/
	...
	for(i=1; i<*numContoursP; i++)	//while( i < 8 ) 
	{
